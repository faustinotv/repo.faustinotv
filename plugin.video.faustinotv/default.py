...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(FaustinoTV,codigo do erro - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(FaustinoTV,codigo do erro - "+str(e.reason)+",10000,"+icon+")")

def get_username_from_user():
    keyboard = xbmc.Keyboard(
        '', AddonTitle + '[COLOR=white]%s[/COLOR]'%", Por favor insira seu nome de usuário", False)
    keyboard.doModal()
...
def get_password_from_user():
    keyboard = xbmc.Keyboard(
        '', AddonTitle + '[COLOR=white]%s[/COLOR]'%", Por favor, insira sua senha", False)
    keyboard.doModal()
    if keyboard.isConfirmed():
        pwd = keyboard.getText()
        return pwd
...
def get_username():
    ...
    try:
        ...
        if USERNAME == "":
            dialog.ok(AddonTitle,'[COLOR=white]%s[/COLOR]'%"Seu nome de usuário não pode estar vazio")
            get_username()
        else:
...
def get_password():
    try:
        ...
        if PASSWORD == "":
            dialog.ok(AddonTitle,'[COLOR=white]%s[/COLOR]'%"Sua senha não pode estar vazia")
            get_password()
        else:
...
def auth_user():
    ...
    P = addon.getSetting('password')
    if U == "" and P == "":
        msg1 ='[COLOR=deepskyblue]%s[/COLOR]'%"[B]Acesso exclusivo para doadores. Entre no grupo do telegram e saiba mais: [COLOR yellow]http://t.me/FaustinoTV[/B][/COLOR]"
		
        msg2 ='[COLOR=white]%s[/COLOR]'%"[B]Deseja inserir usuário e senha agora?[/B]"
        yes_pressed = dialog.yesno(
            AddonTitle + " - Acesso Exclusivo", msg1, msg2,)
        if yes_pressed:
            doLogin()
        else:
            exit()
...
def getSources():
        try:
            addDir('[B][COLOR lime]•[/COLOR] [COLOR yellow]VIP[/COLOR] [COLOR deepskyblue](Bônus)[/COLOR][/B]','url',30,vipicon,FANART,'','','','')
            url = "https://tinyurl.com/r3jbzam"
            getData(url,FANART, data=None)
            if os.path.exists(favorites) == True:
                addDir('[COLOR cyan][B]-- FAVORITOS --[/B][/COLOR]','VIP',4,icon,FANART,'','','','')
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if not name in FAV:
                contextMenu.append(('Adicionar aos Favoritos do FaustinoTV','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#        print 'parentalblockedpin',parentalblockedpin
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                ...
                contextMenu.append(('Adicionar aos Favoritos do FaustinoTV','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
